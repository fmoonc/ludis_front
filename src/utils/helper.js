...
const helpers = {
  ...
  deleteFirstLetter(text) {
    return text.slice(1);
  },
  includesOtherMenus(text) {
    return text.includes("simple-menu") || text.includes("top-menu");
  },
  deleteOtherMenus(text) {
    const partsText = text.split("/");
    return partsText[partsText.length - 1];
  },

  changeFormat(dateRange) {
    alert(dateRange);
